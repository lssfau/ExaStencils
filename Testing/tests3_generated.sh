...
RESULT="$(mktemp --tmpdir=/run/shm test_res_XXXXX.txt)" || {
    echo "========= FAILURE: ID '${ID}': Failed to create temporary file on machine(s) ${SLURM_JOB_NODELIST} in ${SLURM_JOB_NAME}:${SLURM_JOB_ID} (running testcode)." >> "${LOG}"
...
function killed {
  echo "          ??? ID '${ID}': Job ${SLURM_JOB_NAME}:${SLURM_JOB_ID} on machine(s) ${SLURM_JOB_NODELIST} killed; possible reasons: timeout, manually canceled, user login (job is then requeued)  (running testcode)." >> "${LOG}"
  exit 0
}
trap killed SIGTERM
...
if diff -B -w --strip-trailing-cr -I "time"  "${RESULT}" "${EXP_RESULT}" > /dev/null; then
  echo "          OK: ID '${ID}' on machine(s) ${SLURM_JOB_NODELIST} (${SLURM_JOB_NUM_NODES} nodes, ${SLURM_JOB_CPUS_PER_NODE} cores)." >> "${LOG}"
else
  echo "========= FAILED: ID '${ID}' on machine(s) ${SLURM_JOB_NODELIST} (${SLURM_JOB_NUM_NODES} nodes, ${SLURM_JOB_CPUS_PER_NODE} cores): invalid result." >> "${LOG}"
