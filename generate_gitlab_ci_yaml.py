...
def generate_tests(generator_path, path_to_test_config, docker_image_name, outputfile):
    with open(path_to_test_config, 'r') as config_file:
        ...
        content += generate_script_line('git clone https://github.com/stganser/isl.git')
        content += generate_script_line('cd isl')
        content += generate_script_line('mkdir install')
        content += generate_script_line('export ISL_INSTALL="${PWD}/install"')
        content += generate_script_line('./autogen.sh')
        content += generate_script_line('./configure --prefix=${ISL_INSTALL} --with-jni-include=/usr/lib/jvm/java-8-openjdk-amd64/include --with-clang=system')
