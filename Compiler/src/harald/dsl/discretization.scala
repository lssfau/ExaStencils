...
*/
          ...
          s += s"for(int i = 1; i < ${o.name}[lev].x1_-1; i++)\n"
          s += s"for(int j = 1; j < ${o.name}[lev].x2_-1; j++) {\n"
          if (DomainKnowledge.rule_dim == 3)
            s += s"for(int k = 1; k < ${o.sizez}; k++) {\n"
...
*/
          if (DomainKnowledge.fragment_L2.get._2.equals("Regular_Cube")) {
            s += s"c_all_xx = Cube.integrate(${o.weakform});\n"
            ...
            for (i1 <- 0 to anzentr - 1)
              for (j1 <- 0 to anzentr - 1)
                ...
                s += s"${o.name}[0].add(i+${addkoeff(i1)(0)},j+${addkoeff(i1)(1)},k+${addkoeff(i1)(2)},${IdxKnowledge.IdxToStencilEntry(dim, stsize, ListBuffer(addkoeff(j1)(0) - addkoeff(i1)(0), addkoeff(j1)(1) - addkoeff(i1)(1), addkoeff(j1)(2) - addkoeff(i1)(2)))},c_all_xx[${i1}][${j1}]);\n"

          } else {

            s += s"c_all_xx = Quadr.integrate(${o.weakform});\n"

            for (i1 <- 0 to anzentr - 1)
              for (j1 <- 0 to anzentr - 1)
                //                 s += s"A->addvs(i+addkoeffi[i1],j+addkoeffj[i1],1+addkoeffi[j1]-addkoeffi[i1],1+addkoeffj[j1]-addkoeffj[i1],c_all_xx[i1][j1]);\n"
                s += s"${o.name}[lev].add(i+${addkoeff(i1)(0)},j+${addkoeff(i1)(1)},${IdxKnowledge.IdxToStencilEntry(dim, stsize, ListBuffer(addkoeff(j1)(0) - addkoeff(i1)(0), addkoeff(j1)(1) - addkoeff(i1)(1)))},c_all_xx[${i1}][${j1}]);\n"
