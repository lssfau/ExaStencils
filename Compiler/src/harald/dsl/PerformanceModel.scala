...
class PerformanceModel(treel2 : TreeL2) {
  ...
  def estimatePerformance(func: String): (Double, Double) = {
    ...
    for (c <- treel2.Functions) {
      if (c._2.name.equals("VCycle") || c._2.name.equals("FASVCycle")) {
        //println(c._2.name)
    ...
    val bytes : Long = DomainKnowledge.globaldatatype_L2 match { case "Double" => 8 case "ComplexDouble" => 16 case _ => 4}
