...
class ParserL4(tree : TreeL2) extends StandardTokenParsers {
  ...
  def stmt : Parser[AbstractStatement] = ("loop" ~ ident ~ "level" ~ (ident | numericLit) ~ "order" ~ ident ~ "block" ~ numericLit ~ numericLit ~ (stmt.+) ~ "next" ^^ {
    ...
    | ident ~ ("@" ~> numericLit).? ~ "=" ~ expr ~ modifier.? ^^ {
      case id ~ l ~ _ ~ e ~ m => l match {
    ...
    | "repeat" ~ "up" ~ expr ~ (stmt.+) ~ "next" ^^ { case a ~ _ ~ e ~ s ~ f => AbstractRepeat(e, s, "up") }
  ...
  def factor : Parser[AbstractExpression] = ("(" ~> expr <~ ")"
    ...
    | numericLit ^^ { case s => (if (isInt(s)) AbstractIntLiteral(s.toLong) else AbstractFloatLiteral(s.toDouble)) }
