...
object IR_ResolveComplexOps extends DefaultStrategy("Resolve operations with complex expressions") {
    this += new Transformation("resolve", {
      case add @ IR_Addition(args) if(args.forall(a => a.isInstanceOf[IR_ComplexExpression])) =>
      ...
      case sub @ IR_Subtraction(left,right) if(left.isInstanceOf[IR_ComplexExpression] && right.isInstanceOf[IR_ComplexExpression]) =>
        IR_ComplexOperations.sub(left.asInstanceOf[IR_ComplexExpression],right.asInstanceOf[IR_ComplexExpression])
      case Mult @ IR_Multiplication(args) if(args.forall(a => a.isInstanceOf[IR_ComplexExpression])) =>
      ...
      case Div @ IR_Division(ce1,ce2) if(ce1.isInstanceOf[IR_ComplexExpression] && ce2.isInstanceOf[IR_ComplexExpression]) =>
