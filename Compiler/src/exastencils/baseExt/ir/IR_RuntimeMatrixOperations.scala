...
object IR_GenerateBasicMatrixOperations {
  ...
  def loopSetSubmatrixMat(source : IR_VariableAccess, destination : IR_VariableAccess, offset_r : IR_Expression, offset_c : IR_Expression) : IR_Scope = {
 ...
 */
  def schur(in : IR_VariableAccess, blockSize : Int, structureA : String, blockSizeA : Int, out : IR_VariableAccess) : IR_Scope = {
    var debug = false
    var func = IR_Scope(Nil)
    ...
    if (structureA == "Blockdiagonal")
      func.body += IR_GenerateRuntimeInversion.blockdiagonal(A, blockSizeA, A_inv)
    else if (structureA == "Diagonal")
      func.body += IR_GenerateRuntimeInversion.diagonal(A, A_inv)
    else
    ...
    func.body += IR_GenerateBasicMatrixOperations.loopSetSubmatrixMat(S_inv, out, n, n)
  ...
  def inverse(in : IR_VariableAccess, out : IR_VariableAccess, matrixStructure : String) : IR_Scope = {
    ...
    matrixStructure match {
      ...
      case "Blockdiagonal" => blockdiagonal(in, Knowledge.experimental_blocksize, out)
      case "Schur"         => schur(in, Knowledge.experimental_blocksize, Knowledge.experimental_structure_A, Knowledge.experimental_blocksize_A, out)
