...
import exastencils.baseExt.ir.IR_TensorExpression
import exastencils.logger.Logger
import exastencils.prettyprinting.PpStream

/// L4_TensorExpression

case class L4_TensorExpression(
  ...
  override def progress = ProgressLocation(IR_TensorExpression(L4_ProgressOption(datatype)(_.progress), this.rows, this.columns, expressions.flatten.map(_.progress).toArray))
