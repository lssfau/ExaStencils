...
case class L4_ReadField(var filename : L4_Expression, var field : L4_FieldAccess, var condition : Option[L4_Expression] = None) extends L4_Statement {
  ...
  override def progress = ProgressLocation(IR_ReadField(filename.progress, field.progress.fieldSelection, condition.getOrElse(L4_BooleanConstant(true)).progress))
}
...
object L4_ResolveReadFieldFunctions extends DefaultStrategy("Resolve read field function references") {
  this += new Transformation("Resolve", {
    case L4_ExpressionStatement(L4_FunctionCall(L4_UnresolvedFunctionReference("readField", level, offset), args)) =>
