...
//        generateVariant()
   ...
   def pollyTimeSizeTests() = {
    ...
    var nfps : Array[NonFunctionalProperties.Value] = new Array(1)
    nfps(0) =  NonFunctionalProperties.time_perCycle
...
//    testConfigs = FeatureModel.filterConfigurationsAtLeastOne(pTest.allConfigs,ccd.getPoints())
    ...
    var forwardFeatureSelection = new ForwardFeatureSelection(features, 10, testConfigs.toArray[Configuration], NonFunctionalProperties.time_perCycle  )
  ...
  var tuple = new Tuple3[NonFunctionalProperties.Value, Array[FFS_Expression], Jama.Matrix](NonFunctionalProperties.time_perCycle,x._2,x._1) 
  
    var dkt : DomainKnowledgeTests = new DomainKnowledgeTests();
    dkt.write("./../FeatureModel/PollyTests/prediction.csv", pTest.predictConfigs(allConfigs, tuple,  NonFunctionalProperties.time_perCycle))
     println("-----------------------------------------------------------------")
