...
class Configuration() {
  ...
  var nfpValues : scala.collection.mutable.Map[String, Double] = scala.collection.mutable.Map()
  ...
  def readSolutionUnseparated(features : scala.collection.mutable.Map[String, String]) = {
    ...
    features.foreach(x => {
      if (FeatureModel.allFeatures.get(x._1).isEmpty) {
        ...
        if (!currFeature.isNumerical && !currFeature.isXorFeature) {
          if (x._2.equals("true"))
            this.boolFeatures.put(currFeature, true)
          else
            this.boolFeatures.put(currFeature, false)
        } else if (currFeature.isNumerical) {
          this.numericalFeatureValues.put(currFeature, x._2.toDouble)
        } else if (currFeature.isXorFeature) {
          this.xorFeatureValues.put(currFeature, x._2)
        }
      }
    })
