...
//  }
  ...
  def asPropositionalFormula() = {
    ...
    allFeatures.foreach(a =>
      if (a._2.isNumerical) FeatureModel.copris.int(Var(a._1), a._2.minValue.toInt, a._2.maxValue.toInt)
  ...
  /**
    ...
    */
  def addRelationship(copris : jp.kobe_u.copris.sugar.Sugar, f : Feature, subFeatures : scala.collection.mutable.Set[Feature]) = {
    subFeatures.foreach(a =>
      if (a.isOptional)
        // a optional sub-feature of f -->  a => f 
        copris.add(Imp(Bool(a.identifier), Bool(f.identifier)))
