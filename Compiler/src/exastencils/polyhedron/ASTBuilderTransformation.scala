    ...
    return node.getType() match {
      ...
      case isl.AstNodeType.NodeFor =>
        if (node.forIsDegenerate() != 0) {
          ...
          val decl : Statement = VariableDeclarationStatement(it, Some(processIslExpr(node.forGetInit())))
          ...
          val init : Statement = VariableDeclarationStatement(it, Some(processIslExpr(node.forGetInit())))
