...
object Exploration {
  ...
  def completeScheduleGuided(prefix : PartialSchedule, extended : Boolean,
    ...
    val sortedCoeffSpacePoints = filteredCoeffSpacePoints.sorted(new Ordering[Array[Int]] {
      override def compare(x : Array[Int], y : Array[Int]) : Int = {
        var i : Int = 0
        val len : Int = math.min(x.length, y.length)
        while (i < len) {
          val cmp = x(i) - y(i) // values are small: no overflow/underflow
          if (cmp != 0)
            return cmp
          i += 1
        }
    ...
    prefix.newBand()
    completeScheduleGuidedTilable(prefix, sortedCoeffSpacePoints, extended, resultsCallback)
  }
