...
case class SIMD_Blend(var a : IR_Expression, var b : IR_Expression, var immediateValue : IR_Expression) extends SIMD_Expression {
  override def datatype : IR_Datatype = SIMD_RealDatatype
  override def prettyprint(out : PpStream) : Unit = {
    val prec = if (Knowledge.useDblPrecision) 'd' else 's'
    Platform.simd_instructionSet match {
      case "SSE3"         => out << "_mm_blendv" << "_p" << prec << '('
      case "AVX" | "AVX2" => out << "_mm256_blendv" << "_p" << prec << '('
...
case class SIMD_Blendv(var a : IR_Expression, var b : IR_Expression, var mask : IR_Expression) extends SIMD_Expression {
  ...
  override def prettyprint(out : PpStream) : Unit = {
    val prec = if (Knowledge.useDblPrecision) 'd' else 's'
    Platform.simd_instructionSet match {
      case "SSE3"         => out << "_mm_blendv" << "_p" << prec << '(' // TODO: only available for SSE 4.1
      case "AVX" | "AVX2" => out << "_mm256_blendv" << "_p" << prec << '('
      case "AVX512"       => Logger.error("Currently unsupported")
      case "NEON"         => Logger.error("Currently unsupported")
      case "IMCI"         => Logger.error("Currently unsupported")
