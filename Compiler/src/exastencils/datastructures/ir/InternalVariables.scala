...
case class FieldDeviceData(var field : Field, var level : Expression, var slot : Expression, var fragmentIdx : Expression = LoopOverFragments.defIt) extends AbstractFieldData {
...
abstract class FieldFlag extends InternalVariable(true, false, true, true, false) {
  // TODO: check if shared features such as slot handling can be extracted from this class and field handling classes
  var field : Field
  var slot : Expression
  var fragmentIdx : Expression
...
case class HostDataUpdated(var field : Field, var slot : Expression, var fragmentIdx : Expression = LoopOverFragments.defIt) extends FieldFlag {
  override def prettyprint(out : PpStream) : Unit = out << resolveAccess(resolveName, fragmentIdx, NullExpression, if (Knowledge.data_useFieldNamesAsIdx) field.identifier else field.index, field.level, NullExpression)
...
case class DeviceDataUpdated(var field : Field, var slot : Expression, var fragmentIdx : Expression = LoopOverFragments.defIt) extends FieldFlag {
  override def prettyprint(out : PpStream) : Unit = out << resolveAccess(resolveName, fragmentIdx, NullExpression, if (Knowledge.data_useFieldNamesAsIdx) field.identifier else field.index, field.level, NullExpression)
