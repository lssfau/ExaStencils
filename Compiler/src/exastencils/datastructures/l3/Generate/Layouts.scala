...
object Layouts {
  ...
  def addLayouts(printer : java.io.PrintWriter) = {
    ...
    printer.println(s"Layout NoComm< $fieldDatatype > {")
    printer.println(s"\tghostLayers = [ ${genSet(_ => 0)} ]")
    printer.println(s"\tduplicateLayers = [ ${genSet(_ => 1)} ]")
    printer.println(s"}")

    if (Knowledge.l3tmp_genVectorFields) {
      printer.println(s"Layout NoCommScalar< Real > {") // TODO: @coarsest
      printer.println(s"\tghostLayers = [ ${genSet(_ => 0)} ]")
      printer.println(s"\tduplicateLayers = [ ${genSet(_ => 1)} ]")
      printer.println(s"}")
    }

    printer.println(s"Layout CommPartTempBlockable< $fieldDatatype > {")
    ...
    printer.println(s"Layout BasicComm< $fieldDatatype > {")
    printer.println(s"\tghostLayers = [ ${genSet(_ => 1)} ] with communication")
    printer.println(s"\tduplicateLayers = [ ${genSet(_ => 1)} ] with communication")
    printer.println(s"}")

    if (Knowledge.l3tmp_genVectorFields) {
      printer.println(s"Layout BasicCommScalar< Real > {") // TODO: @coarsest
      printer.println(s"\tghostLayers = [ ${genSet(_ => 1)} ] with communication")
      printer.println(s"\tduplicateLayers = [ ${genSet(_ => 1)} ] with communication")
      printer.println(s"}")
    }

    printer.println(s"Layout CommFullTempBlockable< $fieldDatatype > {")
    if (Knowledge.l3tmp_genTemporalBlocking)
    ...
    if (Knowledge.l3tmp_genExtFields) {
      printer.println(s"Layout ExtSolLayout< $fieldDatatype > {")
      printer.println(s"\tghostLayers = [ ${genSet(_ => 0)} ]")
