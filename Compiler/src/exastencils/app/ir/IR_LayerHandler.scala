...
object IR_DefaultLayerHandler extends IR_LayerHandler {
  ...
  override def handle() : Unit = {
    ...
    IR_PreItMatOps.apply()
    var matrixChanged = true
    while (matrixChanged) {
      IR_ExtractMatOps.apply()
      if(IR_ExtractMatOps.results.forall(t => t._2.matches == 0)) matrixChanged = false
      IR_ExtractMatOps.reset()
    }

    matrixChanged = true
    while (matrixChanged) {
      IR_InlineMatOps.apply()
      IR_ResolveUnknownDts.apply()
      IR_MarkResolvableMatOps.apply()
      IR_ResolveMatOps.apply()
      if (
        IR_MarkResolvableMatOps.results.forall(t => t._2.matches == 0)
          && IR_InlineMatOps.results.forall(t => t._2.matches == 0)
          && IR_ResolveMatOps.results.forall(t => t._2.matches == 0)
      ) {
        matrixChanged = false
      }
      IR_InlineMatOps.reset()
      IR_MarkResolvableMatOps.reset()
      IR_ResolveMatOps.reset()
      //IR_GeneralSimplify.doUntilDone()
