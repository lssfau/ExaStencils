  ...
  this += new Transformation("SearchAndReplace", {
    case FunctionCallExpression("diag", args) => args(0) match {
      ...
      case access : StencilFieldAccess => {
        ...
        new FieldAccess(FieldSelection(access.stencilFieldSelection.field, access.stencilFieldSelection.level, access.stencilFieldSelection.slot, Array() /* war Some(0) */, access.stencilFieldSelection.fragIdx), index)
      }
      case _ => {
