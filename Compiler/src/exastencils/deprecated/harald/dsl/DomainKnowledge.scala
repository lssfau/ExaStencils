...
object DomainKnowledge extends ExaKnowledge {
  ...
  var bandwidth_HW : Option[Int] = None
  // in GB/s
  var peak_HW : Option[Int] = None
  // in GFLOPS
  ...
  var stenciltype = "constant"
  // variable, nonlinear
  var vectorentries = 1

  var use_FE = false
  // set to true if one or more stencils require it
  var use_gpu = false
  // set to true if hw is gpu
  var use_MPI = false
  // set to true if nodes > 1
  var use_Openmp = false
  // set to true if cores > 1
