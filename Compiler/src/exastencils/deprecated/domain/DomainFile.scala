...
object DomainFileWriter extends BuildfileGenerator {
  override def write() = {
    val domains = if (Knowledge.domain_useCase != "") IR_DomainCollection.objects.filter { f => f.name != "global" } else IR_DomainCollection.objects
    ...
    printer <<< "domainIdentifier = \"" + domains.map { d => d.name }.mkString(",") + "\""
