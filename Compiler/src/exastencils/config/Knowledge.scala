...
object Knowledge {
  ...
  def domain_canHaveLocalNeighs : Boolean = Knowledge.refinement_enabled || domain_numFragmentsPerBlock > 1 || domain_rect_hasPeriodicity
  // evaluates if fragments can have remote (i.e. different block) neighbors, i.e. if mpi comm is required
  def domain_canHaveRemoteNeighs : Boolean = Knowledge.refinement_enabled || domain_numBlocks > 1 || (mpi_enabled && domain_rect_hasPeriodicity)
