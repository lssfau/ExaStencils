...
object CUDA_ExecutionConfiguration {
  def apply(numBlocks : Array[Long], numThreads : Array[Long]) =
    new CUDA_ExecutionConfiguration(numBlocks.map(n => n : IR_Expression), numThreads.map(n => n : IR_Expression), CUDA_ComputeStream())
...
case class CUDA_ExecutionConfiguration(
    ...
    var stream : CUDA_Stream, // associated stream, default: 0
