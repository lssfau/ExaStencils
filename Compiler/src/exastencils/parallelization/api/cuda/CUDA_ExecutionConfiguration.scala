...
object CUDA_ExecutionConfiguration {
  def apply(numBlocks : Array[Long], numThreads : Array[Long]) =
    new CUDA_ExecutionConfiguration(numBlocks.map(n => n : IR_Expression), numThreads.map(n => n : IR_Expression))
...
case class CUDA_ExecutionConfiguration(
    ...
    var stream : IR_Expression = 0, // associated stream, default: 0
    var sharedMemPerBlock : IR_Expression = 0 // dynamically allocated shared mem in bytes, default: 0
