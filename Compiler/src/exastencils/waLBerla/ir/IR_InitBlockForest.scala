...
case class IR_InitBlockForest() extends IR_WaLBerlaFuturePlainFunction {
  ...
  def someWaLBerlaField = IR_WaLBerlaFieldCollection.objects.collectFirst { case f : IR_WaLBerlaField if f.level == level => f }.get

  def numCellsTotal(d : Int) = if (d < numDims) someWaLBerlaField.layout.layoutsPerDim(d).numInnerLayers else 1
