...
case class IR_WaLBerlaCPUCommScheme(var wbField : IR_WaLBerlaField, var slot : IR_Expression) extends IR_WaLBerlaCommScheme {
  ...
  def createUniformPackInfo() = {
    val packInfoType = if (Knowledge.waLBerla_useRefinement) "field::refinement::PackInfo" else "field::communication::PackInfo"
    ...
    make_shared(s"$packInfoType< $templateArgs >", blockDataID)
  }
