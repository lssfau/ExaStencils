...
object IR_WaLBerlaReplaceCommIVs extends IR_WaLBerlaReplacementStrategy("Replace comm IV accesses to wb counterparts") {
  ...
  this += Transformation("Replace", {
    case _ @ IR_IV_CommBuffer(field, send, size, neighIdx, concurrencyId, indexOfRefinedNeighbor, fragmentIdx) if inWaLBerlaScope(collector)        =>
      IR_WaLBerlaCommBuffer(field, send, size, neighIdx, concurrencyId, indexOfRefinedNeighbor, fragmentIdx)
    case _ @ IR_IV_CommBufferBasePtr(field, send, size, neighIdx, concurrencyId, indexOfRefinedNeighbor, fragmentIdx) if inWaLBerlaScope(collector) =>
      IR_WaLBerlaCommBufferBasePtr(field, send, size, neighIdx, concurrencyId, indexOfRefinedNeighbor, fragmentIdx)
    case _ @ IR_IV_CommBufferIterator(field, send, neighIdx, concurrencyId, indexOfRefinedNeighbor, fragmentIdx) if inWaLBerlaScope(collector)      =>
    ...
    case _ @ IR_IV_RemoteReqOutstandingNoField(send, neighIdx, concurrencyId, indexOfRefinedNeighbor, fragmentIdx) if inWaLBerlaScope(collector) =>
