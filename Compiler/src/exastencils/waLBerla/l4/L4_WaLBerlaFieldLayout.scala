...
case class L4_WaLBerlaFieldLayout(
  ...
  override def createDuplicate() : L4_WaLBerlaFieldLayout = {
    L4_WaLBerlaFieldLayout(name, level, numDimsGrid, Duplicate(datatype), waLBerlaLayout,
      Duplicate(ghostLayers), communicatesGhosts, Duplicate(duplicateLayers), communicatesDuplicated)
  }
  ...
  def innerPoints = L4_ConstIndex(Array.fill(numDimsGrid)(1)) // unused for wb layouts
