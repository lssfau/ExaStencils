...
case class ExpKernel(var identifier : String,
  ...
  def init() : Unit = {
    ...
    if (smemCanBeUsed) {
      if (executionDim < parallelDims) {
        globalThreadId = globalThreadId :+ new VariableAccess(loopVariables(executionDim), Some(IntegerDatatype))
      }
  ...
  def evaluateAccessesForSharedMemory() : Unit = {
    ...
    for (fa <- fieldToFieldAccesses if !foundSomeAppropriateField) {
      ...
      if (fieldAccesses.size > 1 && leftDeviationFromBaseIndex.deep == rightDeviationFromBaseIndex.deep && requiredMemoryInByte < availableSharedMemory && isSameRadius) {
        ...
        fieldName = name
        fieldBaseIndex = new MultiIndex(baseIndex)
  ...
  def addSmemStmtsSpatialBlocking(statements : ListBuffer[Statement], conditionAccess : VariableAccess) : ListBuffer[Statement] = {
    ...
    zDimLoopBody += new ConditionStatement(conditionAccess, zDimLoopBodyPart1)
