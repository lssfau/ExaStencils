...
object IR_FunctionCall {
  ...
  = new IR_FunctionCall(HACK_IR_UndeterminedFunctionAccess(functionName, IR_UnitDatatype), args.to[ListBuffer])
  @deprecated("Used for backwards compatibility - to be removed", "22.09.16")
  def apply(functionName : String, args : ListBuffer[IR_Expression])
  = new IR_FunctionCall(HACK_IR_UndeterminedFunctionAccess(functionName, IR_UnitDatatype), args)
}
