...
object BoundaryCondition {
  def apply(eq: Equation) = {
    ...
    left match {
      ...
      case _ => throw new IllegalArgumentException(s"""|Allowed boundary conditions are:
                                                       |\t- Dirichlets\n\t- Neumann.""".stripMargin)
    }
  }
}
...
final class ExprStencil(val entries: Seq[ExprStencilParam]) extends ExprStencilParam {
  ...
  override def toString = dimCount match {
    ...
    case _ => { val stringList = for(i <- 0 until len) yield "["+(i)+"]:\n"+entries(i).toString
      stringList.mkString("\n")
    }
  }
...
case class Exp(left: MathTree, right: MathTree) extends BinaryNode {def base = left; def power = right}
...
final object ReservedNames {
  ...
  def fieldsToStringSet() = (this.getClass.getDeclaredFields :\ Set[String]()) {
    (field, set) => field.setAccessible(true); field.get(this) match {
      case x: (Any,Any) => ( set
  ...
  def _fieldStringsToMap = (this.getClass.getDeclaredFields :\ Map[String,String]()) {
    (field, map) => field.setAccessible(true); field.get(this) match {
      //case x: (Any,Any) => map +
