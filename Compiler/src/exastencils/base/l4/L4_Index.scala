...
case class L4_ExpressionIndex(override var indices : Array[L4_Expression]) extends L4_Index with L4_ArrayBasedIndex[L4_Expression] {
  ...
  override def +(that : L4_Index) : L4_ExpressionIndex = {
    that match {
...
case class L4_ConstIndex(override var indices : Array[Int]) extends L4_Index with L4_ArrayBasedIndex[Int] {
  ...
  override def +(that : L4_Index) = {
    that match {
      case that : L4_ConstIndex      => L4_ConstIndex(this, that, _ + _)
      case that : L4_ExpressionIndex => L4_ExpressionIndex(this.toExpressionIndex, that, _ + _)
    }
  }
  override def -(that : L4_Index) = {
    that match {
