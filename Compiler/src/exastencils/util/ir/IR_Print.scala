...
object IR_PrintOnRoot {
  def apply(toPrint : IR_Expression*) = new IR_PrintOnRoot(toPrint.to[ListBuffer])
}

case class IR_PrintOnRoot(var toPrint : ListBuffer[IR_Expression], var stream : IR_VariableAccess = IR_VariableAccess("std::cout", IR_UnknownDatatype)) extends IR_Statement with IR_Expandable {
...
object IR_PrintPerRank {
  def apply(toPrint : IR_Expression*) = new IR_PrintPerRank(toPrint.to[ListBuffer])
}

case class IR_PrintPerRank(var toPrint : ListBuffer[IR_Expression], var stream : IR_VariableAccess = IR_VariableAccess("std::cout", IR_UnknownDatatype)) extends IR_Statement with IR_Expandable {
  override def expand() : Output[IR_Statement] = {
    val rankIndicator = if (Knowledge.mpi_enabled)
      ListBuffer(IR_StringConstant("["), MPI_IV_MpiRank, IR_StringConstant("] "))
    else
      ListBuffer()
    ...
    if (toPrint.isEmpty) {
      ...
      IR_Print(stream,
        rankIndicator ++ toPrint.view.flatMap { e => ListBuffer(e, IR_StringConstant(" ")) }.to[ListBuffer] :+  IR_Print.endl)
    }
  }
}
