  ...
  override def generateFct() : IR_PlainFunction = {
    ...
    ifBody += IR_ForLoop(
      ...
      ListBuffer[IR_Statement](
        ...
        IR_IfCondition(
          callExtFunction("VisIt_CommandMetaData_alloc", IR_AddressOf(cmd)) EqEq visitOkay,
          ListBuffer[IR_Statement](
            callExtFunction("VisIt_CommandMetaData_setName", cmd, IR_ArrayAccess(commandNames, IR_FieldIteratorAccess(0))),
            callExtFunction("VisIt_SimulationMetaData_addGenericCommand", md, cmd))))
    )

    fctBody += IR_VariableDeclaration(md, visitInvalidHandle)
    fctBody += IR_IfCondition(
      callExtFunction("VisIt_SimulationMetaData_alloc", IR_AddressOf(md)) EqEq visitOkay,
      ifBody
    )
