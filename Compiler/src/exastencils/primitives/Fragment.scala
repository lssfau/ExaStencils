...
//    addSync(fields);
//
//    functions += (new ConnectLocalElement().toString_cpp);
//    functions += (new ConnectRemoteElement().toString_cpp);
//    functions += (new SetupBuffers(fields).toString_cpp);
    ...
    for (m <- members)
      s += s"${m}";
   	*/
    //s += s"public:\n${(for (m <- members) yield (m))}"
    //s += s"public:\n${members.toString}"

    /// HACK
//    {
//      val writer = new PrintWriter(new File(s"C:/Users/sisekuck/Documents/Visual Studio 2010/Projects/ExaStencils_DSL/Poisson3D/Primitives/Fragment3DCube_TEMP.h"));
//      writer.write(FragmentClass.toString_cpp);
//      writer.close();
//    }
    var i = 0;
    for (f <- functions) {
