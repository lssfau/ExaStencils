...
abstract class L2_LeveledKnowledgeCollection[L2_Type <: L2_LeveledKnowledgeObject[L3_Type] : TypeTag, L3_Type <: L3_KnowledgeObject[_]] extends L2_KnowledgeCollection {
  ...
  def existsDecl(identifier : String, level : Option[L2_LevelSpecification]) : Boolean = {
    if (level.isEmpty) Logger.error(s"Missing level specification for $identifier")
    level.get match {
      case L2_SingleLevel(lvl) => existsDecl(identifier, lvl)
  ...
  def addDeclared(identifier : String, level : Option[L2_LevelSpecification]) : Unit = {
    if (level.isEmpty) Logger.error(s"Missing level specification for $identifier")
    level.get match {
      case L2_SingleLevel(lvl) => addDeclared(identifier, lvl)
      case other               => Logger.error(s"Unsupported level specification for $identifier: ${ other.prettyprint() }")
    }
  }
