...
object Grid_AxisAlignedVariableWidth extends Grid {
  ...
  override def invokeResolve(specialField : SpecialFieldAccess) : Expression = {
    ...
    fctName.substring(fctName.length() - 2) match {
      case "_x" => {
        ...
        method.invoke(this, specialField.level : java.lang.Integer, specialField.index, specialField.arrayIndex, 0 : Integer).asInstanceOf[Expression]
      }
      case "_y" => {
        val method = this.getClass().getMethods.find(_.getName == fctName.substring(0, fctName.length - 2)).get
        method.invoke(this, specialField.level : java.lang.Integer, specialField.index, specialField.arrayIndex, 1 : Integer).asInstanceOf[Expression]
      }
      case "_z" => {
        val method = this.getClass().getMethods.find(_.getName == fctName.substring(0, fctName.length - 2)).get
        method.invoke(this, specialField.level : java.lang.Integer, specialField.index, specialField.arrayIndex, 2 : Integer).asInstanceOf[Expression]
      }
      case _ => {
        val method = this.getClass().getMethods.find(_.getName == fctName).get
        method.invoke(this, specialField.level : java.lang.Integer, specialField.index, specialField.arrayIndex).asInstanceOf[Expression]
      }
    }
  }
  ...
  // direct accesses
  def get_node_pos(level : Int, index : MultiIndex, arrayIndex : Option[Int], dim : Int) = {
  ...
  def get_stag_cv_width(level : Int, index : MultiIndex, arrayIndex : Option[Int], dim : Int) = {
    val field = FieldCollection.getFieldByIdentifier(s"stag_cv_width_${dimToString(dim)}", level).get
    FieldAccess(FieldSelection(field, field.level, 0, arrayIndex), projectIdx(index, dim))
  }

  // compound accesses
  def get_cell_width(level : Int, index : MultiIndex, arrayIndex : Option[Int], dim : Int) = {
  ...
  def get_cell_center_to_face(level : Int, index : MultiIndex, arrayIndex : Option[Int], dim : Int) = {
